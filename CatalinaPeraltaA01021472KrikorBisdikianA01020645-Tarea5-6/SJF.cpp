...
void SJF::nextArrives(std::list<Proceso> & process_queue, Proceso & next)
{
    ...
    {
        for (std::list<Proceso>::iterator it = process_queue.begin(); it != process_queue.end(); it++)
        {
            if (next<=*it)
            {
...
void SJF::incrementWait(std::list<Proceso> & process_queue){
...
void SJF::getProcess(std::queue<Proceso> & process_start, double & quantum){
    std::vector<std::string> text;
    bool p=true;
    
    std::string line;
    std::ifstream myfile;
    myfile.open("/Users/iCaty/Documents/TEC/QuintoSemestre/SistemasOperativos/procesos.txt");
    if (myfile.is_open())
    {
        while ( getline (myfile,line))
        {
            if(!p){
                ...
                process_start.push(Proceso(text[0],stof(text[1]), stof(text[2])));
            }else {
                quantum = stof(line);
            }
            p=false;
